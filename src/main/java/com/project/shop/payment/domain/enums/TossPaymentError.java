...
public enum TossPaymentError {
    ...
    TossPaymentError(int status, String message) {
        this.status = status;
    ...
    public static TossPaymentError get(String errorCode){
        for(TossPaymentError error : TossPaymentError.values()){
            if(error.name().equals(errorCode)){
                return error;
            }
        }
        return UNKNOWN;
